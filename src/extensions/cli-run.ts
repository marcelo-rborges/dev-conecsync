...
module.exports = (toolbox: GluegunToolbox) => {
  toolbox.run = async (props: string) => {
    ...
    const CONFIG_ONLINE_STATUS_BARCODE: boolean =
      configJson?.onlineStatus?.barcodes ?? false
    const CONFIG_ONLINE_STATUS_NO_BARCODE: boolean =
      configJson?.onlineStatus?.noBarcodes ?? false
    // const SANDBOX: boolean = !!get(config, 'sandbox');
    LOGS.push(['Conexão DB em uso', CONFIG_DB || '-'])
    LOGS.push(['Pasta CSVS', CONFIG_CSVS || '-'])
    LOGS.push([
      'onlineStatus.barcode',
      !!CONFIG_ONLINE_STATUS_BARCODE ? 'True' : 'False',
    ])
    LOGS.push([
      'onlineStatus.noBarcode',
      !!CONFIG_ONLINE_STATUS_NO_BARCODE ? 'True' : 'False',
    ])
    ...
    if (!CONFIG_DB && !CONFIG_CSVS) {
      ...
      print.success(
        'SOLUÇÃO: Indique um tipo de conexão de banco de dados desejado (db:) ou uma pasta para arquivos .csv (csvs:) em "/config/config.json".',
      )
    ...
    if (!!CONFIG_DB && !SUPPORTED_ALL.includes(CONFIG_DB)) {
      ...
      print.success(
        'SOLUÇÃO: Indique um tipo VÁLIDO de conexão de banco de dados (db:) em "/config/config.json".',
      )
      // print.info('conecsync config (exibe arquivo de configuração.');
      print.divider()
      toolbox.configuracao()
    ...
    if (
      ...
      print.success(
        'SOLUÇÃO: Indique pelo menos uma origem de dados para sincronização em "/config/origens/*.json".',
      )
      toolbox.origens()
      print.divider()
      return
    ...
    // destinos
    const MERCADEIRO_LOJAS: any[] = (get(mercadeiro, 'lojas') || []).filter(
      (l: any) => get(l, 'id') && get(l, 'token'),
    )
    ...
    if (
      ...
      print.success(
        'SOLUÇÃO: Indique as lojas (ids e tokens) nos projetos cujas apis deseja acessar em "/config/destinos/*.json".',
      )
      print.divider()
      toolbox.destinos()
      return
